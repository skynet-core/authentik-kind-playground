...
if [ ! -f "$keys/private.pem" ] || [ ! -f "$keys/public.pem" ]; then
	printf "\033[32m Generating self-signed certificate for Traefik... \033[0m"
...
printf "\033[32m Deploying Traefik CRD ... \033[0m"
kubectl apply -f "$DIR/crds.yaml"

printf "\033[32m Deploying ServiceAccount, ClusterRole and ClusterRoleBinding for Traefik ... \033[0m"
kubectl apply -f "$DIR/rbac.yaml"

printf "\033[32m Persistent volume... \033[0m"
kubectl apply -f "$DIR/pvc.yaml"

printf "\033[32m Deploying Traefik Service... \033[0m"
kubectl apply -f "$DIR/svc.yaml"

printf "\033[32m Deploying Traefik Gateway... \033[0m"
kubectl apply -f "$DIR/gateway.yaml"

printf "\033[32m Deploying Traefik... \033[0m"
kubectl apply -f "$DIR/deployment.yaml"

kubectl wait --timeout=5m -n default \
	deployment.apps/traefik --for=condition=Available

if [ ! -f "$DIR/dashboard-auth.secret" ]; then
...
printf "\033[32m Deploying Traefik Dashboard... \033[0m"
